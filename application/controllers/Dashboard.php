...
class Dashboard extends CI_Controller {
    ...
    function index() {
        ...
        $data['stokpc'] = $this->getstokpc();
        $data['pcdigunakan'] = $this->m_stok->getpcdigunakan();
        $data['laptopdigunakan'] = $this->m_stok->getlaptopdigunakan();
        $data['invrusak'] = $this->m_stok->getallrusak();
        $this->template->display('dashboard/index', $data);
    }
   
    function getstokpc(){
        $data = $this->m_stok->getstokpc();
        $jmlstok =0;
        foreach($data as $s){
            $stok = chek_stok($s->kode_barang);
            $jmlstok +=$stok;            
        }
        return $jmlstok;
    }

...
class Maintenance extends CI_Controller {
    ...
    public function view_tanggal() {  
        ...
        //     $ambildata=$this->m_maintenance->getsemua_tanggal($tglawal,$tglakhir); 
        // }else{
        $ambildata=$this->m_maintenance->getall_tanggal($tglawal,$tglakhir); 
        // }                            
        $no=1;
        foreach($ambildata as $r) { 
            if ($r->status =="OPEN"){
	...
	function detail() { 
        ...
        if($maintenance['nama_kategori']=='Laptop'){
            ...
            $inv=$this->m_printer->get_inv($maintenance['no_inventaris'])->row_array();
        }elseif ($maintenance['nama_kategori']=='Device') {
            $inv=$this->m_network->get_inv($maintenance['no_inventaris'])->row_array();
        }
        $data['inv']=$inv;                                         
        $data['recordall'] = $this->m_maintenance->getall($id)->row_array();
